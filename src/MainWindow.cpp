...
unsigned int			mcaster1Thread = 0;

mcaster1Globals			*g[MAX_ENCODERS];
...
extern "C"
{
int startMcaster1Thread(void *obj) {
	CMainWindow *pWindow = (CMainWindow *) obj;
	pWindow->startMcaster1(-1);
...
{
int startSpecificMcaster1Thread(void *obj) {
	int		enc = (int) obj;

	/*
	 * CMainWindow *pWindow = (CMainWindow *)obj;
	 */
	int		ret = pWindow->startMcaster1(enc);
	time_t	currentTime;
	...
	for(int i = 0; i < gMain.gNumEncoders; i++) {
		if(g[i]->forcedDisconnect) {
			...
			if(timediff > timeout) {
				...
				_beginthreadex(NULL,
							   0,
							   (unsigned(_stdcall *) (void *)) startSpecificMcaster1Thread,
							   (void *) i,
							   0,
							   &mcaster1Thread);
//Begin patch multiple cpu
...
VOID CALLBACK MetadataTimer(HWND hwnd, UINT uMsg, UINT idEvent, DWORD dwTime) {
	...
	if(!strcmp(gMain.externalMetadata, "URL")) {
		...
		TRY
		{
			CInternetSession	session("Mcaster1 DSP Encoder");
 ...
 * PortAudio delivers paFloat32 interleaved samples directly. */
                            ...
                            PaStreamCallbackFlags statusFlags, void *userData)
{
	...
	if (!gLiveRecording || !inputBuffer)
		return paContinue;

	int nch   = 2;
	int srate = 48000;

	handleAllOutput((float *)inputBuffer, (int)framesPerBuffer, nch, srate);
...
int startRecording(int deviceIndex) {
	...
	if (deviceIndex < 0) {
		...
		if (deviceIndex == paNoDevice) {
			pWindow->generalStatusCallback((char *)"No PortAudio input device available!");
			return 0;
		}
	}
	...
	if (!devInfo || devInfo->maxInputChannels < 1) {
		pWindow->generalStatusCallback((char *)"Selected device has no input channels!");
		return 0;
	}
	...
	PaError err = Pa_OpenStream(&g_paStream, &inputParams, NULL,
	                            48000.0, 512, paNoFlag,
	                            paRecordCallback, NULL);
	if (err != paNoError) {
		char msg[255];
		sprintf(msg, "PortAudio open error: %s", Pa_GetErrorText(err));
		pWindow->generalStatusCallback(msg);
		g_paStream = NULL;
		return 0;
	}
	...
	if (err != paNoError) {
		char msg[255];
		sprintf(msg, "PortAudio start error: %s", Pa_GetErrorText(err));
		pWindow->generalStatusCallback(msg);
		Pa_CloseStream(g_paStream);
		g_paStream = NULL;
		return 0;
	...
	char statusMsg[512];
	sprintf(statusMsg, "Recording from: %s", devInfo->name);
	pWindow->generalStatusCallback(statusMsg);

	g_paDeviceIndex = deviceIndex;
	gLiveRecording  = true;
	return 1;
}

/* CMainWindow dialog */
const char	*kpcTrayNotificationMsg_ = "Mcaster1 DSP Encoder";

CMainWindow::CMainWindow(CWnd *pParent /* NULL */ ) :
	CResizableDialog(CMainWindow::IDD, pParent),
	bMinimized_(false),
	pTrayIcon_(0),
...
BEGIN_MESSAGE_MAP(CMainWindow, CResizableDialog)
...
int CMainWindow::startMcaster1(int which) {
	if(which == -1) {
		for(int i = 0; i < gMain.gNumEncoders; i++) {
			if(!g[i]->weareconnected) {
				setForceStop(g[i], 0);
...
void CMainWindow::OnConnect() {
	...
	if(!connected) {
		_beginthreadex(NULL, 0, (unsigned(_stdcall *) (void *)) startMcaster1Thread, (void *) this, 0, &mcaster1Thread);
//Begin patch multiple cpu
HANDLE hProc = GetCurrentProcess();//Gets the current process handle
      DWORD procMask;
      DWORD sysMask;
...
void CMainWindow::OnAddEncoder() {
	...
	g[orig_index] = (mcaster1Globals *) malloc(sizeof(mcaster1Globals));

	memset(g[orig_index], '\000', sizeof(mcaster1Globals));
	...
	mcaster1_init(g[orig_index]);
}

BOOL CMainWindow::OnInitDialog() {
	CResizableDialog::OnInitDialog();

	//SetWindowText("Mcaster1 DSP Encoder");
	...
	for(int i = 0; i < gMain.gNumEncoders; i++) {
		if(!g[i]) {
			g[i] = (mcaster1Globals *) malloc(sizeof(mcaster1Globals));
			memset(g[i], '\000', sizeof(mcaster1Globals));
		}
	...
	/* Enumerate input devices via PortAudio */
	Pa_Initialize();
	int numDevices = Pa_GetDeviceCount();
	m_CurrentInputCard = Pa_GetDefaultInputDevice(); /* fallback */
	const char *savedDevice = getWindowsRecordingDevice(&gMain);

	for (int a = 0; a < numDevices; a++) {
		const PaDeviceInfo *devInfo = Pa_GetDeviceInfo(a);
		if (!devInfo || devInfo->maxInputChannels < 1) continue;
		m_RecCardsCtrl.AddString(devInfo->name);
		if (savedDevice && strcmp(savedDevice, "") != 0) {
			if (!strcmp(savedDevice, devInfo->name)) {
				m_RecCards = devInfo->name;
				m_CurrentInputCard = a;
			}
		} else {
			if (a == Pa_GetDefaultInputDevice()) {
				m_RecCards = devInfo->name;
				m_CurrentInputCard = a;
			}
		}
	...
	// ResizableLib anchors — controls that stretch or pin to edges
	AddAnchor(IDC_METADATA,              TOP_LEFT,    TOP_RIGHT);   // metadata bar stretches
	AddAnchor(IDC_MANUALEDIT_METADATA,   TOP_LEFT,    TOP_LEFT);    // Edit button stays left
	AddAnchor(IDC_METER,                 TOP_LEFT,    TOP_RIGHT);   // VU meter stretches
	AddAnchor(IDC_RECVOLUME,             TOP_LEFT,    TOP_RIGHT);   // volume slider stretches
	AddAnchor(IDC_RECCARDS,              TOP_LEFT,    TOP_RIGHT);   // device combo stretches
	AddAnchor(IDC_RECDEVICES,            TOP_RIGHT,   TOP_RIGHT);   // rec channel stays right
	AddAnchor(IDC_LIVEREC_GROUP,         TOP_LEFT,    TOP_RIGHT);   // Live Recording box stretches
	AddAnchor(IDC_LIVEREC,               TOP_LEFT,    TOP_LEFT);
	AddAnchor(IDC_AUTOCONNECT,           TOP_LEFT,    TOP_LEFT);
	AddAnchor(IDC_CONNECT,               TOP_LEFT,    TOP_LEFT);
	AddAnchor(IDC_ADD_ENCODER,           TOP_LEFT,    TOP_LEFT);
	AddAnchor(IDC_VUONOFF,               TOP_RIGHT,   TOP_RIGHT);
	AddAnchor(IDC_ENCODERS,              TOP_LEFT,    BOTTOM_RIGHT); // main list fills space
	AddAnchor(IDC_STATIC_STATUS,         BOTTOM_LEFT, BOTTOM_RIGHT); // status bar pins bottom
...
void CMainWindow::OnAboutHelp() {
	...
	/* TODO: Add your command handler code here */
	char	loc[2046] = "";
	sprintf(loc, "%s\\%s", m_currentDir, "mcaster1dspencoder.chm");
...
void CMainWindow::SetupTrayIcon() {
	if(bMinimized_ && (pTrayIcon_ == 0)) {
		...
		pTrayIcon_->Create(0, nTrayNotificationMsg_, "Mcaster1 DSP Encoder", hIcon_, IDR_SYSTRAY);
...
void CMainWindow::OnSelchangeReccards()
{
	...
	/* Find the PortAudio device index for the selected card name */
	int numDevices = Pa_GetDeviceCount();
	for (int a = 0; a < numDevices; a++) {
		const PaDeviceInfo *devInfo = Pa_GetDeviceInfo(a);
		if (!devInfo || devInfo->maxInputChannels < 1) continue;
		if (!strcmp(selectedCard, devInfo->name)) {
			m_CurrentInputCard = a;
			break;
		}
	}

	/* Repopulate channel selector for new device */
	m_RecDevicesCtrl.ResetContent();
	m_RecDevicesCtrl.AddString("Stereo");
	m_RecDevicesCtrl.AddString("Left Only");
	m_RecDevicesCtrl.AddString("Right Only");
...
{
	/* Let ResizableLib reposition/resize all anchored controls first */
