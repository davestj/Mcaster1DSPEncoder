; =============================================================================
; Mcaster1 DSP Encoder — foobar2000 Component Installer
;
; Pre-build:   .\generate_version.ps1
; Compile:     makensis mcaster1_foobar_setup.nsi
; Output:      mcaster1_foobar_<version>_setup.exe
;
; Component:   $INSTDIR\components\foo_mcaster1.dll
; Runtime DLLs: $INSTDIR\   (foobar2000 root — matches deploy-foobar.ps1)
; =============================================================================
...
; Branding
; ---------------------------------------------------------------------------
!define APP_NAME      "Mcaster1 DSP Encoder (foobar2000 Component)"
!define COMPANY_NAME  "Mcaster1"
!define PUBLISHER     "David St. John"
!define PRODUCT_URL   "https://mcaster1.com/encoder.php"
!define UNINST_KEY    "Software\Microsoft\Windows\CurrentVersion\Uninstall\Mcaster1DSP_foobar2000"
!define UNINST_ROOT   "HKLM"

!define SRC_DIR       "foobar2000\Release"
...
OutFile           "mcaster1_foobar_${VERSION}_setup.exe"
...
; ---------------------------------------------------------------------------
; Splash + auto-detect foobar2000 path
; ---------------------------------------------------------------------------
Function .onInit
    InitPluginsDir
    ...
    ; foobar2000 registry detection
    StrCpy $DetectedPath ""
    ; App Paths key — "Path" value is the directory
    ReadRegStr $DetectedPath HKLM \
        "SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\foobar2000.exe" "Path"
    ${If} $DetectedPath == ""
        ReadRegStr $DetectedPath HKLM "SOFTWARE\WOW6432Node\foobar2000" "Location"
    ${EndIf}
    ${If} $DetectedPath == ""
        ReadRegStr $DetectedPath HKLM "SOFTWARE\foobar2000" "Location"
    ${EndIf}
    ${If} $DetectedPath == ""
        StrCpy $DetectedPath "$PROGRAMFILES\foobar2000"
    ${EndIf}
...
Function PageDetectDir
    !insertmacro MUI_HEADER_TEXT "foobar2000 Install Location" \
        "Confirm the path to your foobar2000 installation."

    nsDialogs::Create 1018
    Pop $0

    ${NSD_CreateLabel} 0 0 100% 40u \
        "foobar2000 was detected at the path shown below.$\r$\nVerify it is correct, or Browse to select a different folder.$\r$\n$\r$\nfoo_mcaster1.dll will be placed in the components sub-folder; runtime DLLs go to the foobar2000 root."
    Pop $0
...
Function PageDetectDirLeave
    ...
    ${If} $INSTDIR == ""
        MessageBox MB_OK|MB_ICONEXCLAMATION "Please select a valid foobar2000 install directory."
        Abort
    ${EndIf}
FunctionEnd
...
; Install section
; ---------------------------------------------------------------------------
Section "Mcaster1 foobar2000 Component" SecMain
    SectionIn RO

    SetOutPath "$INSTDIR\components"
    File "${SRC_DIR}\foo_mcaster1.dll"
    ...
    WriteRegStr   ${UNINST_ROOT} "${UNINST_KEY}" "UninstallString" "$INSTDIR\uninstall_mcaster1_foobar.exe"
    ...
    WriteUninstaller "$INSTDIR\uninstall_mcaster1_foobar.exe"
SectionEnd
...
Section "Uninstall"
    ...
    Delete "$INSTDIR\uninstall_mcaster1_foobar.exe"
