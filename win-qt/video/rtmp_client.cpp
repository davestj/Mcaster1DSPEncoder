...
bool RtmpClient::tcp_connect(const std::string& host, uint16_t port)
{
    ...
    int opt = 1;
#ifndef _WIN32
    /* SO_NOSIGPIPE (macOS doesn't support MSG_NOSIGNAL; Windows has no SIGPIPE) */
    ::setsockopt(sock_fd_, SOL_SOCKET, SO_NOSIGPIPE, &opt, sizeof(opt));
#endif

    /* TCP_NODELAY for low latency */
    ::setsockopt(sock_fd_, IPPROTO_TCP, TCP_NODELAY, reinterpret_cast<const char*>(&opt), sizeof(opt));
...
bool RtmpClient::tcp_send(const uint8_t* data, size_t len)
{
    ...
    while (sent < len) {
        ...
        int n = ::send(sock_fd_, buf, static_cast<int>(len - sent), 0);
        if (n <= 0) {
...
bool RtmpClient::tcp_recv(uint8_t* buf, size_t len)
{
    size_t received = 0;
    while (received < len) {
        auto* dst = reinterpret_cast<char*>(buf + received);
        int n = ::recv(sock_fd_, dst, static_cast<int>(len - received), 0);
        if (n <= 0) {
