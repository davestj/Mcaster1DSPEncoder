...
// ---------------------------------------------------------------------------
// CoreAudioDevice — comprehensive macOS device info from CoreAudio
// ---------------------------------------------------------------------------
struct CoreAudioDevice {
    uint32_t    device_id;          // AudioObjectID
    std::string name;
    std::string uid;                // persistent unique ID (CFString)
    int         input_channels;
    int         output_channels;
    double      sample_rate;
    bool        is_default_input;
...
// Enumerate all CoreAudio devices (input, output, aggregate, virtual).
// Returns comprehensive device list including BlackHole, Loopback, etc.
